...
int main(int argc, char* argv[]){
	...
	std::string jsonfile="example.json";

	//parse json input file
	std::ifstream ifs("example.json", std::ifstream::in);
	...
	std::cout<<"d top level object contains "<<d.HasMember("camera")<<" members:"<<std::endl;
	...
	for (SizeType i = 0; i < shapes.Size(); i++)
		{// Retrieve values through GetXXX() functions
	        printf("shapes[%d] = %s radius: %f %f\n", i,
	        		shapes[i].GetObject()["type"].GetString(),
					shapes[i].GetObject()["radius"].GetFloat(),
					shapes[i].GetObject()["center"][0].GetFloat());
			Value& ar = shapes[i].GetObject()["center"];
			SizeType s = ar.Size();
			for (SizeType j = 0; j < s; j++ ){
				std::cout << ar[j].GetFloat() << std::endl;
